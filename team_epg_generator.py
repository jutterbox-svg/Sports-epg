...
def build_team_xmltv(team_entries: list, events_by_team_id: dict, durations: dict, now=None) -> Element:
    ...
    for entry in team_entries:
        ...
        # First pass: parse every event for this team into a normalized
        # record with its TRUE kickoff time (used for the live/finished
        # logic and for the human-readable date in the title - never
        # altered), so we can sort chronologically before laying out
        # display start/stop times.
        parsed = []
        for event in events_by_team_id.get(team_id, []):
            actual_start = parse_event_start_utc(event)
            if actual_start is None:
                continue
            actual_stop = datetime.fromtimestamp(actual_start.timestamp() + minutes * 60, tz=timezone.utc)
        ...
        # (even if its real kickoff is days away), consecutive entries
        # chain directly onto each other with no blank space between,
        # and the last known entry is stretched well into the future so
        # the grid never goes empty before the next refresh cycle picks
        # up newer data.
